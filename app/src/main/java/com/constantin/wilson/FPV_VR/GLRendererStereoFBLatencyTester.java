...
/**
 * Created by Constantin on 04.05.2017.
 */

public class GLRendererStereoFBLatencyTester implements GLSurfaceViewFB.IRendererEGL14/*,SurfaceTexture.OnFrameAvailableListener*/{
    ...
    public GLRendererStereoFBLatencyTester(Context context){
        mContext =context;
    ...
    private void drawRightEye(){
        clearCount+=0.1;
        if(clearCount>2){clearCount=0;}
        clearColor=(int)clearCount;
        switch (clearColor){
    ...
    private void updateHeadPos(){
        ...
        /*float[] xyz= mCGTracker.getPosXYZ();
        float diff=lastX-xyz[0];
        if(diff<0){diff=-diff;}
        if(diff>1){
            clearColor++;
            if(clearColor>2){
